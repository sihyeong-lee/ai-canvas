       ...
       파이썬_법령상세URL  파이썬_판례상세URL 파이썬_해석상세URL
         ...
         [노드 12]         [노드 13]       [노드 14]
          API_법령본문      API_판례본문    API_해석본문
                ...
                [노드 15] 데이터 연결_근거통합
                                ↓
           ★NEW [노드 16] 파이썬_근거표준화   (Evidence 스키마 변환)
                                ↓
           ★NEW [노드 17] 파이썬_근거스코어링  (신뢰도 점수 부여)
                                ↓
...
| 14 | API_해석본문 | API | - |
| 15 | 데이터 연결_근거통합 | 전처리 | - |
| 16 | 파이썬_근거표준화 | 전처리 | **★NEW** |
| 17 | 파이썬_근거스코어링 | 전처리 | **★NEW** |
| 18 | 에이전트 프롬프트_답변생성 | API | **프롬프트 v2, 모델 변경** |
| 19 | 파이썬_검열게이트 | 전처리 | **v2 로직, re.compile 제거** |
| 20 | 데이터 조건 분기 | 데이터 | - |
| 21 | 프롬프트_차단응답 | API | - |
| 22 | 에이전트로 전달 | 데이터 | - |
...
> **Phase 3 (내부DB 연동 시)**: 입력4에 `데이터셋_내부징계DB` 또는 `API_내부징계DB조회` 결과를 추가 연결합니다.
...
### 노드 17: 파이썬_근거스코어링 ★NEW

- **카테고리**: 전처리 > 파이썬 스크립트
- **입력**: 노드 16(파이썬_근거표준화) 결과
...
| 데이터 연결_근거통합 | 통합 결과 | 파이썬_근거표준화 | 입력 데이터셋 |
